...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    title: `Terms of Service - ${product.name} | XYVN`,
    description: `Terms of Service for ${product.name}. Read our terms and conditions for using the service.`,
  };
}
  ...
  const formatContent = (content: string) => {
    ...
    lines.forEach((line, index) => {
      ...
      if (trimmedLine.startsWith('# ')) {
        ...
        flushList();
        // Handle bold text within paragraphs and all caps warnings
        let formattedText = trimmedLine.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
        // Style legal disclaimers
        const isDisclaimer = trimmedLine.startsWith('TO THE MAXIMUM EXTENT');
        elements.push(
          <p 
            key={index} 
            className={`mb-4 leading-relaxed ${isDisclaimer ? 'text-gray-400 uppercase text-sm font-medium' : 'text-gray-300'}`}
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="prose prose-invert max-w-none">
          <div className="p-8 md:p-12 rounded-3xl bg-white/[0.02] border border-white/[0.06] backdrop-blur-xl">
            {formatContent(product.termsOfService)}
          </div>
        </div>
